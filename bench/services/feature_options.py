...
from bench.schemas import EditorContext, FeatureOption, FeatureOptionsRequest, FeatureOptionsResponse
    ...
    async def generate(self, request: FeatureOptionsRequest) -> FeatureOptionsResponse:
        ...
        if "(test)" in request.prompt.lower():
            ...
            return FeatureOptionsResponse(
                ...
                options=_serialize_options(suggestions),
            )
        ...
        system_prompt = (
            ...
            "Bench can preview and apply generatedCode directly in the editor. When you can infer target files, return generatedCode using one or more file-aware sections in this exact format:\n"
            "### relative/path.ext\n```language\n...code...\n```\n"
            "Use workspace-relative paths only. Prefer the smallest useful file set, but return multiple files when that is the cleanest implementation.\n"
            "If the active file already contains the target function, class, route, or placeholder, align generatedCode to that file and symbol.\n"
            "Do not include explanations inside generatedCode; keep explanations in summary, implementationPlan, and tradeoffs.\n"
            "For authenticated endpoint requests, return Python FastAPI code only. Each generatedCode value must be a complete candidate_target.py file defining create_app() that returns a FastAPI app with GET /health and authenticated GET /protected routes. /protected must require Authorization: Bearer test-token, return 401 when missing, return 403 when wrong, and return 200 JSON when valid.\n"
            "Do not include metrics; Bench will populate measured results after Docker evaluation.\n"
...
def _serialize_options(options: list[FeatureOption]) -> list[dict[str, object]]:
    return [option.model_dump(by_alias=True) for option in options]
...
def _load_options_json(cleaned: str) -> dict[str, object]:
    ...
    for candidate in (cleaned, _escape_control_chars_inside_strings(cleaned)):
        try:
    ...
    if payload is None:
        ...
        if not recovered:
            raise ValueError(
                "Cerebras response was not valid JSON (the output was likely truncated)."
            )
...
def _recover_complete_suggestions(content: str) -> list[dict[str, object]]:
    """Salvage fully-formed suggestion objects from a truncated JSON response."""
    ...
    for index in range(bracket + 1, len(content)):
        ...
        if char == "{":
            ...
            if depth == 0 and obj_start != -1:
                fragment = content[obj_start : index + 1]
                try:
                    objects.append(
                        json.loads(_escape_control_chars_inside_strings(fragment))
                    )
                except json.JSONDecodeError:
                    pass
                obj_start = -1
...
def _build_assistant_message(option_count: int) -> str:
    return (
        ...
        "Context was condensed locally to limit token usage. Run the candidates to populate measured metrics."
    )
...
def _build_test_options(prompt: str) -> list[FeatureOption]:
    ...
    return [
        FeatureOption.model_validate(item)
        for item in raw
    ]
